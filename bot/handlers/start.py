...
async def cmd_start(message: Message) -> None:
    ...
    async with async_session() as session:
        ...
        cnt = await count_user_accounts(session, message.from_user.id)

    await message.answer(
        t("start.welcome", lang, name=message.from_user.first_name),
        reply_markup=get_start_keyboard(
            user_id=message.from_user.id, lang=lang, accounts_count=cnt,
        ),
        parse_mode="HTML",
    )
...
async def back_to_menu(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    async with async_session() as session:
        ...
        cnt = await count_user_accounts(session, callback.from_user.id)

    await callback.message.edit_text(
        t("start.welcome", lang, name=callback.from_user.first_name),
        reply_markup=get_start_keyboard(
            user_id=callback.from_user.id, lang=lang, accounts_count=cnt,
        ),
        parse_mode="HTML",
    )
...
async def open_admin_panel(callback: CallbackQuery) -> None:
    ...
    if callback.from_user.id not in settings.admin_id_list:
        await callback.answer(t("admin.no_access", lang), show_alert=True)
        return
